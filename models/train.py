...
# parser.add_argument('--data_dir', type=str, default='/app/datas/val')
...
# Add weight decay to prevent overfitting and NaN
...
def safe_einsum_reconstruction(pred, f0_t, f1_t, f2_t, z0, f_0, f_1, f_2):
    """Safe reconstruction with proper normalization and dimension handling"""
    # Clip factor tensors to prevent explosion
...
def train(epoch, model, loader, device):
    ...
    with tqdm(loader, desc=f'Epoch {epoch:05d}',
        ...
        for core, f_0, f_1, f_2, rgb, seg in loader:
            ...
            n = core.shape[0] # due to distributed training, batch size is not always the same as args.batch_size
            f_0 = f_0.to(device, dtype=torch.bfloat16)
            f_1 = f_1.to(device, dtype=torch.bfloat16)
            f_2 = f_2.to(device, dtype=torch.bfloat16)
            context = torch.cat([rgb, seg], dim=1).to(device, dtype=torch.bfloat16) # channel-wise concat
            # Time embedding
            ...
            # Normalize data to prevent explosion - more reasonable clipping
            ...
            # Loss calculation with better numerical stability
            def stable_loss_fn(x, y):
                diff = x - y
                # Clip differences to prevent explosion
                diff = torch.clamp(diff, -10.0, 10.0)
                # Use MSE loss with proper scaling
                return (diff ** 2).mean() * 1000  # Scale up the loss
            ...
            # Gradient clipping - moderate for better training
...
def test(epoch, model, device):
    ...
    for batch_idx_t, (names_t, core_t, f0_t, f1_t, f2_t, rgb_t, seg_t) in enumerate(test_loader):
        ...
        with torch.no_grad():
            for i, j in tqdm(zip(reversed(seq_t), reversed(seq_next_t)), desc='Testing', total=len(seq_t), disable=not ISRANK_0):
                ...
                # z0_t_t = (zt_t - (1 - at_t).sqrt() *  pred) / (at_t.sqrt()+1e-6)
                # zt_t = at_next_t.sqrt() * z0_t_t + (1 - at_next_t).sqrt() * pred
                
            # 생성된 이미지: 예측된 core + 예측된 팩터들
    ...
    if ISRANK_0:
        ...
        # 올바른 비교를 위해 같은 변수에서 가져오기
        re = final_hsi_t[batch_idx_t*n:(batch_idx_t+1)*n]  # 이미 numpy로 변환된 것 사용
        re_gt = re_gt.detach().to(torch.float32).cpu().numpy()
        # 생성된 이미지와 원본 이미지 비교 (Tucker 분해 재구성 결과)
        for re_i, re_gt_i, name in zip(re, re_gt, names_t):
            # Better data range calculation for more realistic metrics
            ...
            # 생성된 이미지만 저장 (512개 밴드 모두 표시)
            # 16x32 = 512개 밴드를 모두 보이도록
